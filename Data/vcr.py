...
from .data import ( TxtTokLmdb, DetectFeatLmdb,TxtLmdb, get_ids_and_lens, pad_tensors,get_gather_index,DetectFeatTxtTokDataset)
...
class QRA_VcrDataset(VcrDetectFeatTxtTokDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # assert self.task != "qa,qar",\
        #     "loading training dataset with each task separately"
    ...
    def _get_input_ids(self, txt_dump):
        ...
        input_ids_as = txt_dump['input_ids_as']
        input_ids_rs = txt_dump['input_ids_rs']
        ra_label = txt_dump['qar_target']
        assert ra_label >= 0, "answer_label < 0"
        input_ids_gt_a = [self.txt_db.sep] + copy.deepcopy(
            input_ids_rs[ra_label])
        type_ids_gt_a = [3] * len(input_ids_gt_a)
        type_ids_q += type_ids_gt_a
        input_ids_q += input_ids_gt_a
        input_ids_for_choices = input_ids_as
        return input_ids_q, input_ids_for_choices, type_ids_q
...
class QR_VcrDataset(VcrDetectFeatTxtTokDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # assert self.task != "qa,qar",\
        #     "loading training dataset with each task separately"
    ...
    def _get_input_ids(self, txt_dump):
        ...
        input_ids_q = txt_dump['input_ids']
        type_ids_q = [0]*len(input_ids_q)
        trg = txt_dump['input_ids_rs'][txt_dump['rationale_label']]
        input_ids_as = txt_dump['input_ids_as']
        input_ids_as_all=[]
        #把所有答案拼在问题后面，用sep隔开
        #用2作为type
        ...
        return input_ids_q, trg, type_ids_q
    ...
    def __getitem__(self, i):
        ...
        img_feat, img_pos_feat, num_bb = self._get_img_feat(
            example['img_fname'][0], example['img_fname'][1])
        input_ids_q, trg, type_ids_q = self._get_input_ids(
            example)
        ...
        outs.append(
            (input_ids, txt_type_ids,
             img_feat, img_pos_feat,
             attn_masks, trg,trg_len,qid,label_raw))
        return tuple(outs)

def QR_vcr_collate(inputs):
    (input_ids, txt_type_ids, img_feats,
     img_pos_feats, attn_masks, targets,trg_len,qids,labels_raw) = map(list, unzip(concat(inputs)))
...
class QR_gpt_VcrDataset(VcrDetectFeatTxtTokDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # assert self.task != "qa,qar",\
        #     "loading training dataset with each task separately"
    ...
    def __getitem__(self, i):
        ...
        input_ids = [self.txt_db.cls_] + copy.deepcopy(input_ids_q) +  [self.txt_db.sep]
        trg= '<|endoftext|> '+example['raw_rs'][example['qar_target']]+'<|endoftext|>'
        txt_type_ids = [0] + type_ids_q+[2]
        attn_masks = torch.ones(
            len(input_ids) + num_bb, dtype=torch.long)
        input_ids = torch.tensor(input_ids)
...
def QR_gpt_vcr_collate(inputs):
    (input_ids, txt_type_ids, img_feats,
     img_pos_feats, attn_masks, targets,qids) = map(list, unzip(concat(inputs)))
...
class QR_t5_VcrDataset(VcrDetectFeatTxtTokDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # assert self.task != "qa,qar",\
        #     "loading training dataset with each task separately"
    ...
    def __getitem__(self, i):
        ...
        input_ids = [self.txt_db.cls_] + copy.deepcopy(input_ids_q) +  [self.txt_db.sep]
        trg= '<s> '+example['raw_rs'][example['qar_target']]+'</s>'
        txt_type_ids = [0] + type_ids_q+[2]
        attn_masks = torch.ones(
            len(input_ids) + num_bb, dtype=torch.long)
        input_ids = torch.tensor(input_ids)
...
class QR_t5_txt_VcrDataset(VcrDetectFeatTxtTokDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # assert self.task != "qa,qar",\
        #     "loading training dataset with each task separately"
...
def QR_t5_txt_vcr_collate(inputs):
    (input_ids, txt_type_ids, img_feats,
     img_pos_feats, attn_masks, targets,qids) = map(list, unzip(concat(inputs)))
