...
from modeling.modeling_sequence_align_encoder1 import ChunkAlign_dec5_4_wo_chual_reasoning_constrain
...
def eval_nlp_scores_toknize(pred, gt, verbose=False):
    """
    ...
    gts = {}
    res = {}
    # 原文不做tokenzier,应该是因为输入的是id
    # for imgId in range(len(pred)):
    #     gts[imgId] = gt[imgId]
    #     res[imgId] = pred[imgId]

    for imgId in range(len(pred)):
        gts[imgId] = [{'caption': gt[imgId][0]}]
        res[imgId] = [{'caption': pred[imgId][0]}]
...
def main():
    ...
    parser.add_argument("--ir", type=str,
                        default='./Oscar/oscar/output/SNLI_base_freeze/checkpoint-1-34000-score0.3296-nlg_score-0.4047-acc-0.8145-blue-0.2992-meteor-0.1906-rouge-0.2775-spice-0.3252-/',
    ...
    model = ChunkAlign_dec5_4_wo_chual_reasoning_constrain(oscar_model, gpt_model, tokenizer, gpt_tokenizer,
                                                           args.beam_size,
                                                           args.repeat_penalty,
    ...
    model_file = os.path.join(args.ir, 'model.pth')
    ...
    with open(os.path.join(args.ir, file_name), 'w') as f:
        for line in pred:
            f.write(line)
            f.write('\n')
    ...
    for idx in range(len(gt_json)):
        if gt_json[idx] != gt[idx]:
            ...
            print(gt_json[idx])
            print(gt[idx])
    score = eval_nlp_scores_toknize(pred, gt_json)
    ...
    workbook.save(os.path.join(args.ir, file_name))
    ...
    file_name = 'SNLI-acc-{:.4f}-nlg_score-{:.4f}-bs-{}-rep-{}-len-{}-con-{}-stop-{}.json'.format(acc, nlg_score,
                                                                                                  ...
                                                                                                  args.constrained,
                                                                                                  args.stop_words_index)
    with open(os.path.join(args.ir, file_name), 'w') as json_file:
        json_file.write(result_str)

    with open(os.path.join(args.ir, 'total_result.json'), 'a') as json_file:
