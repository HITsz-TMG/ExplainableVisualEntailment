...
from modeling.modeling_vqa_x_con_copy import ChunkAlign_CLS_dec5_4_con_copy_mask_5, SeqBertImgModel
from transformers import BertTokenizerFast, BertConfig
import pickle
from Data.VQAChunkAlign import VQA_ChunkAlign_prefix_Dataset
...
id2label = pickle.load(open('./Oscar/datasets/vqa/trainval_label2ans.pkl', 'rb'))
    ...
    with torch.no_grad():
        for step, batch in enumerate(test_dataloader):
            inputs = {'input_ids': batch['input_ids'], 'token_type_ids': batch['token_type_ids'],
                      ...
                      'expl_ids': batch['gpt_ids'], 'gather_index': batch['gather_index'],
            ...
            golden_expls = batch['expl_list']
            for q_id, que, la, oscar_match, pre, expl_list, output in zip(q_ids, ques, batch['label'], matched_0,
                                                                             pres, golden_expls, outputs):
                tmp_res = []
                que_str = tokenizer.decode(que.tolist(), skip_special_tokens=True)
                o_exp_str = model.dec_toker.decode(output, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                if oscar_match.item() == True:
                    gts_exp[index] = expl_list
                    res_exp[index] = [o_exp_str]
                    index += 1
                    tmp_res.append(
                        {'hypo': que_str, 'oscar_match': str(oscar_match.item()),
                         'golden_rel': id2label[la.max(dim=-1)[1]], 'pre': id2label[pre],
                         'golden_expl': expl_list, 'output_expl': o_exp_str})
                else:
                    tmp_res.append(
                        {'hypo': que_str, 'oscar_match': str(oscar_match.item()),
                         'golden_rel': id2label[la.max(dim=-1)[1]], 'pre': id2label[pre],
                         'golden_expl': expl_list, 'output_expl': ''})
                if q_id in result_dict.keys():
                    result_dict[q_id].append(tmp_res[0])
                else:
                    result_dict[q_id] = tmp_res
            pbar(step=step,
...
def main():
    ...
    parser.add_argument("--vqa_x_example_file_train",
                        default="./Oscar/datasets/vqa/VQA-X/train_x.json", type=str)
    parser.add_argument("--vqa_x_example_file_dev",
                        default="./Oscar/datasets/vqa/VQA-X/val_x.json", type=str)
    parser.add_argument("--vqa_x_example_file_test",
                        default="./Oscar/datasets/vqa/VQA-X/test_x.json",
                        type=str)
    parser.add_argument("--vqa_x_feat_file_train",
                        default="./Oscar/datasets/vqa/VQA-X/feat_train.pkl", type=str)
    parser.add_argument("--vqa_x_feat_file_dev",
                        default="./Oscar/datasets/vqa/VQA-X/feat_val.pkl", type=str)
    parser.add_argument("--vqa_x_feat_file_test",
                        default="./Oscar/datasets/vqa/VQA-X/feat_test.pkl",
                        type=str)
    parser.add_argument("--vqa_x_ans2label",
                        default="./Oscar/datasets/vqa/trainval_ans2label.pkl",
                        type=str)

    parser.add_argument("--vqa_x_chunk_mask_train",
                        default="./Oscar/datasets/vqa/VQA-X/ChunkMaskTrain_v4.pkl",
                        type=str)
    parser.add_argument("--vqa_x_chunk_mask_dev",
                        default="./Oscar/datasets/vqa/VQA-X/ChunkMaskDev_v4.pkl", type=str)
    parser.add_argument("--vqa_x_chunk_mask_test",
                        default="./Oscar/datasets/vqa/VQA-X/ChunkMaskTest_v4.pkl",
                        type=str)
    ...
    parser.add_argument("--enc_pretrain_model_dir", type=str,
                        default='./Oscar/oscar/output/VQA_X_ChunkAlign_enc4/checkpoint-28-3250-acc-0.8568/',
                        help="Model directory for evaluation.")
    parser.add_argument("--output_dir", default='./output/VQA_X_ChunkAlign_dec5_4', type=str, required=False,
    ...
    parser.add_argument("--eval_model_dir", type=str,
                        default='./Oscar/oscar/output/VQA_X_ChunkAlign_dec5_4_con_copy_mask_5_1_1/checkpoint-27-6400-score0.3193-nlg_score-0.3709-acc-0.8609-blue-0.2314-meteor-0.2247-rouge-0.4512-cider_exp-0.7398-spice-0.2074-/',
                        help="Model directory for evaluation.")
    ...
    parser.add_argument("--epoch_begin", default=20, type=int)
    ...
    parser.add_argument(
        ...
        help="")
    parser.add_argument(
        '--stop_words_index', default=0, type=int
    )
    ...
    model = ChunkAlign_CLS_dec5_4_con_copy_mask_5(oscar_model, seq_model, gpt_model, tokenizer, gpt_tokenizer,
    ...
    test_dataset = VQA_ChunkAlign_prefix_Dataset(tokenizer, gpt_tokenizer, args.vqa_x_example_file_test,
                                                 args.vqa_x_chunk_mask_test, args.vqa_x_ans2label,
                                                 args.vqa_x_feat_file_test, is_train=False)
    ...
    file_name = 'gen_0-rep-{}-len-{}-con-{}-stop-{}.txt'.format(args.repeat_penalty, args.length_penalty,
                                                                args.constrained, args.stop_words_index)
    with open(os.path.join(args.eval_model_dir, file_name), 'w') as f:
        for line in gt:
            f.write(str(line[0]))
    ...
    file_name = 'gen_1-rep-{}-len-{}-con-{}-stop-{}.txt'.format(args.repeat_penalty, args.length_penalty,
                                                                args.constrained, args.stop_words_index)
    with open(os.path.join(args.eval_model_dir, file_name), 'w') as f:
        for line in gt:
            f.write(str(line[1]))
    ...
    file_name = 'gen_2-rep-{}-len-{}-con-{}-stop-{}.txt'.format(args.repeat_penalty, args.length_penalty,
                                                                args.constrained, args.stop_words_index)
    with open(os.path.join(args.eval_model_dir, file_name), 'w') as f:
        for line in gt:
            f.write(str(line[2]))
