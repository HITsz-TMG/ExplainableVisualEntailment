...
from modeling.modeling_vcr_chunkalign_v10 import ChunkAlign_CLS_enc4_align, SeqBertImgModel
from transformers import BertTokenizerFast, BertConfig
from transformers import AdamW, get_linear_schedule_with_warmup, get_constant_schedule_with_warmup
import pickle
from Data.VCRChunkAlign import VCR_ChunkAlign_Dataset_align
...
def train(args, train_dataloader, val_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss_cls, matched_0, align_loss, correct, total_sum = model(**inputs)
            ...
            global_align_loss += align_loss.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                pbar(step=new_step % pbar_len,
                     info={'Epoch': epoch, 'oscar_match_0': n_correct_qa_0 / n_examples, 'loss': global_loss / new_step,
                           'cls_loss_0': global_cls_loss / new_step, 'align_loss': global_align_loss / new_step,
                           'align_mathc': correct_num / total_num})
...
def main():
    ...
    parser.add_argument("--output_dir", default='./output/VCR_ChunkAlign_enc1_4_align_loss', type=str, required=False,
    ...
    model = ChunkAlign_CLS_enc4_align(oscar_model, seq_model, num_labels=4)
    if args.do_test:
        model_file = os.path.join(args.eval_model_dir, 'model.pth')
        model.load_state_dict(torch.load(model_file))
    ...
    if args.do_train:
        train_dataset = VCR_ChunkAlign_Dataset_align(tokenizer, gpt_tokenizer, args.vcr_example_file_train,
                                                     args.vcr_chunk_mask_train,
                                                     args.vcr_feat_file_train)
        train_dataloader = build_dataloader(train_dataset, True, args)
        val_dataset = VCR_ChunkAlign_Dataset_align(tokenizer, gpt_tokenizer, args.vcr_example_file_dev,
