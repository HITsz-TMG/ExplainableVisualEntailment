...
from modeling.modeling_snli_con_copy import ChunkAlign_dec5_4_con_copy_mask_5_1, SeqBertImgModel
...
def save_checkpoint(model, tokenizer, args, epoch, iteration, optimizer, scheduler, num_trial=10, acc=0.0, blue=0.0,
    ...
    nlg_score = (blue + meteor_exp + rouge_exp + spice_exp + cider_exp) / 5
    global_score = acc * nlg_score
    checkpoint_dir = op.join(args.output_dir,
                             'checkpoint-{}-{}-score{:.4f}-nlg_score-{:.4f}-acc-{:.4f}-blue-{:.4f}-meteor-{:.4f}-rouge-{:.4f}-cider_exp-{:.4f}-spice-{:.4f}-'.format(
                                 epoch, iteration, global_score, nlg_score, acc, blue, meteor_exp, rouge_exp, cider_exp,
                                 spice_exp))
    if not is_main_process():
        return checkpoint_dir
    mkdir(checkpoint_dir)
    ...
    if res_str is not None:
        file_name = 'SNLI-acc-{:.4f}-blue-{:.4f}.json'.format(acc, blue)
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {'input_ids': batch['input_ids'], 'token_type_ids': batch['token_type_ids'],
                      ...
                      'gather_index': batch['gather_index'], 'pre_keywords': batch['pre_keywords'],
                      'offsets': batch['offsets'], 'chunk_attention_mask': batch['chunk_attention_mask'],
                      'gpt_labels': batch['gpt_labels']}
...
def main():
    ...
    parser.add_argument("--seq_model_name_or_path",
                        ...
                        type=str, required=False,
                        help="Path to pre-trained model or model type.")
    parser.add_argument("--output_dir", default='./output/SNLI_dec5_4_con_copy_mask_5_1', type=str, required=False,
    ...
    gpt_model = gpt_model_class(config=gpt_config)
    ...
    oscar_model.resize_token_embeddings(len(tokenizer) + 1)
    ...
    model = ChunkAlign_dec5_4_con_copy_mask_5_1(oscar_model, gpt_model, seq_model, tokenizer, gpt_tokenizer,
                                                args.beam_size)
    # 加载预训练encoder
    ...
    if args.do_train:
        train_dataset = SNLISeqAlignChunkDataset_v7_prompt(tokenizer, gpt_tokenizer, args.SNLI_example_file_train,
        ...
        val_dataset = SNLISeqAlignChunkDataset_v7_prompt(tokenizer, gpt_tokenizer, args.SNLI_example_file_dev,
