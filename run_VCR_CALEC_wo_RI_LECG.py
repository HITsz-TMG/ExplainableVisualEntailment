...
from modeling.modeling_vcr_chunkalign_v10 import ChunkAlign_CLS_dec5_4_wo_reasoning, SeqBertImgModel
            ...
            for img_key, que, an, la, oscar_match, pre, gen_expl, golden_expl in zip(img_keys, ques, ans, label,
                ...
                if oscar_match.item() == True:
                    ...
                    try:
                        ...
                        gen_expl_str = model.dec_toker.decode(gen_expl[:e_rtnl_index], skip_special_tokens=False)
                    except:
                        gen_expl_str = model.dec_toker.decode(gen_expl, skip_special_tokens=False)
...
def main():
    ...
    parser.add_argument("--output_dir", default='./output/VCR_dec5_4_wo_reasoning', type=str, required=False,
    ...
    parser.add_argument("--enc_pretrain_model_dir", type=str,
                        default='./Oscar/oscar/output/VCR_ChunkAlign_enc1_4_wo_reasoning/checkpoint-10-31000-acc-0.7284/',
                        help="Model directory for evaluation.")
    ...
    model = ChunkAlign_CLS_dec5_4_wo_reasoning(oscar_model, seq_model, gpt_model, gpt_tokenizer, num_labels=4)
    # 加载预训练encoder
