...
from modeling.modeling_sequence_align_encoder1 import ChunkAlign_dec5_4_wo_reasoning, SeqBertImgModel
...
def main():
    ...
    parser.add_argument("--seq_model_name_or_path",
                        ...
                        type=str, required=False,
                        help="Path to pre-trained model or model type.")
    parser.add_argument("--output_dir", default='./output/SNLI_CAS_wo_reasoning_dec5_4', type=str, required=False,
    ...
    parser.add_argument("--enc_pretrain_model_dir", type=str,
                        default='./Oscar/oscar/output/SNLI_ChunkAlign_CLS_enc4_wo_reasoning/checkpoint-4-7500-acc-0.8144/',
                        help="Model directory for evaluation.")
    ...
    model = ChunkAlign_dec5_4_wo_reasoning(oscar_model, gpt_model, seq_model, gpt_tokenizer, args.beam_size,
                                           max_hypo=args.max_hypo_len)
    # 加载预训练encoder
        ...
        for imgId in result_dic.keys():
            for i in range(len(result_dic[imgId])):
                if result_dic[imgId][i]['oscar_match'] == 'True':
                    ...
                    gt.append(result_dic[imgId][i]['golden_expl'].lower())
        ...
        for imgId in result_dic.keys():
            for i in range(len(result_dic[imgId])):
                if result_dic[imgId][i]['oscar_match'] == 'True':
                    pred.append([result_dic[imgId][i]['output_expl'].lower()])
                    gt.append([result_dic[imgId][i]['golden_expl'].lower()])
                    gt_json.append([test_json[imgId][i].lower()])
