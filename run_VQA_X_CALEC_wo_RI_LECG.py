...
from modeling.modeling_vqa_x_chunkalign_v10 import ChunkAlign_CLS_dec5_4_wo_reasoning, SeqBertImgModel
...
def train(args, train_dataloader, val_dataset, model, tokenizer):
    ...
    # seq align
    # encoder冻结
    enc = ['seq_enc', 'global_enc', 'cls_ensemble', 'cls_layer', 'classifier']
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            exp_loss, loss_cls, matched_0 = model(**inputs)
            n_correct_qa_0 += matched_0.sum().item()
            loss = loss_cls + exp_loss
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                pbar(step=new_step % pbar_len,
                     info={'Epoch': epoch, 'oscar_match_0': n_correct_qa_0 / n_examples, 'loss': global_loss / new_step,
                           'cls_loss_0': global_cls_loss / new_step, 'exp_loss': global_exp_loss / new_step})
                ...
                if global_step % args.valid_steps == 0 and epoch >= args.epoch_begin:
                    ...
                    checkpoint_dir = save_checkpoint(model, tokenizer, args, epoch, global_step, optimizer, scheduler,
                                                     acc=acc, blue=blue_1, res_str=result_str, meteor_exp=meteor_exp,
...
def eval(args, test_dataloader, model, tokenizer):
    ...
    with torch.no_grad():
        for step, batch in enumerate(test_dataloader):
            ...
            for img_key, que, la, oscar_match, pre, expl_list, output in zip(img_keys, ques, batch['label'], matched_0,
...
def main():
    ...
    parser.add_argument("--enc_pretrain_model_dir", type=str,
                        default='./Oscar/oscar/output/VQA_X_ChunkAlign_enc4_wo_reasoning/checkpoint-29-3350-acc-0.8328/',
                        help="Model directory for evaluation.")
    parser.add_argument("--output_dir", default='./output/VQA_X_ChunkAlign_dec5_4_wo_reasoning', type=str,
                        required=False,
    ...
    model = ChunkAlign_CLS_dec5_4_wo_reasoning(oscar_model, seq_model, gpt_model, gpt_tokenizer,
    ...
    if args.do_train:
        train_dataset = VQA_ChunkAlign_prefix_Dataset(tokenizer, gpt_tokenizer, args.vqa_x_example_file_train,
        ...
        val_dataset = VQA_ChunkAlign_prefix_Dataset(tokenizer, gpt_tokenizer, args.vqa_x_example_file_dev,
                                                    args.vqa_x_chunk_mask_dev, args.vqa_x_ans2label,
                                                    args.vqa_x_feat_file_dev, is_train=False)
        ...
        for imgId in result_dic.keys():
            for i in range(len(result_dic[imgId])):
                if result_dic[imgId][i]['oscar_match'] == 'True':
                    pred.append(result_dic[imgId][i]['output_expl'].lower())
        ...
        for key in result_dic.keys():
            for j in range(len(result_dic[key])):
                ...
                sheet.write(index, 5, pre)
                sheet.write(index, 6, match)
        file_name = 'SNLI-acc-{:.4f}-nlg_score-{:.4f}-bs-{}.xls'.format(acc, nlg_score, args.beam_size)
        workbook.save(os.path.join(args.eval_model_dir, file_name))
