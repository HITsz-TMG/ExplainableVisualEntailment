...
from modeling.modeling_vcr_chunkalign_v10 import ChunkAlign_CLS_dec5_4, SeqBertImgModel
            ...
            for img_key, que, an, la, oscar_match, pre, gen_expl, golden_expl in zip(img_keys, ques, ans, label,
                ...
                if oscar_match.item() == True:
                    ...
                    try:
                        ...
                        gen_expl_str = model.dec_toker.decode(gen_expl[:e_rtnl_index], skip_special_tokens=False).replace("<|det", "").replace(
                            "|>", "")
                    except:
                        gen_expl_str = model.dec_toker.decode(gen_expl, skip_special_tokens=False).replace("<|det", "").replace(
                            "|>", "")
...
def main():
    ...
    model = ChunkAlign_CLS_dec5_4(oscar_model, seq_model, gpt_model, gpt_tokenizer, num_labels=4)
    # 加载预训练encoder
