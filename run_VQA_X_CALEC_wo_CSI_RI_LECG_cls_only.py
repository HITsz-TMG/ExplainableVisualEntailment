...
from modeling.modeling_vqa_x_chunkalign_v10 import BaseLine_cls_xe
...
def train(args, train_dataloader, val_dataset, model, tokenizer):
    ...
    n_examples = 0
    new_step = 0
    num_answers = train_dataloader.dataset.num_answers
...
def main():
    ...
    parser.add_argument("--model_name_or_path",
                        default='./Oscar/pretrained_models/image_captioning/pretrained_large/pretrained_large/checkpoint-1410000/',
                        type=str, required=False,
                        help="Path to pre-trained model or model type.")
    parser.add_argument("--output_dir", default='./output/VQA_X_BaseLine_cls_xe', type=str, required=False,
    ...
    parser.add_argument("--epoch_begin", default=15, type=int)
    ...
    model = BaseLine_cls_xe(oscar_model, args.beam_size, num_labels=len(id2label))
    if args.do_test:
        model_file = os.path.join(args.eval_model_dir, 'model.pth')
        model.load_state_dict(torch.load(model_file))
