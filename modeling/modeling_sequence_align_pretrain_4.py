...
class SeqAlign_pretrain_v4(nn.Module):
    def __init__(self, seq_enc, config):
        super(SeqAlign_pretrain_v4, self).__init__()
    ...
    def forward(self, input_ids, img_feat, total_label, img_mask, input_mask,
        ...
        attn_weight = torch.stack(seq_outputs[2][-6:], dim=1).sum(1).sum(1)[:, :hypo_len, hypo_len:]
