...
class SeqAlign_pretrain_v3(nn.Module):
    def __init__(self, seq_enc, chunk, config):
        super(SeqAlign_pretrain_v3, self).__init__()
        self.dropout = nn.Dropout(0.3)
        self.chunk = chunk
        self.id2label = chunk.config.id2label
    ...
    def forward(self, input_ids, img_feat, total_label, img_mask, input_mask,
                ...
                align_pos=None):
        hypo_len = input_ids.size(1)
        logits = self.chunk(input_ids, attention_mask=input_mask[:, :hypo_len]).logits
