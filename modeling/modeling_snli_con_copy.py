...
from oscar.modeling.modeling_bert_gpt import BeamSearchScorer
...
class ChunkAlign_dec5_4_con_copy_mask_5_1(nn.Module):
    def __init__(self, global_enc, dec, seq_enc, enc_toker, dec_toker, beam_size, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_dec5_4_con_copy_mask_5_1, self).__init__()
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
                ...
                offsets=None, chunk_attention_mask=None, gpt_labels=None, gather_index=None, pre_keywords=None):
        hypo_len = input_ids.size(1)
        batch_size = input_ids.size(0)
    ...
    def evaluate(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        outputs = torch.full((batch_size, self.max_len), fill_value=self.dec_toker.pad_token_id,
                             dtype=int).cuda()
        one_hot = torch.zeros(batch_size, hypo_len, self.enc_vocab_num).cuda()
        one_hot = one_hot.scatter_(dim=-1, index=input_ids.unsqueeze(-1), value=1)[:, 1:, :]
        one_hot = one_hot.unsqueeze(1).repeat(1, 3, 1, 1).reshape(batch_size, -1, self.enc_vocab_num)
        ...
        for i in range(matched_0.size(0)):
            if matched_0[i] == torch.tensor(True):
                ...
                for index in range(self.max_len - 1):
