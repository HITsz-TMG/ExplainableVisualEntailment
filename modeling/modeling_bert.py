...
class BertImgModel_gtToken(BertPreTrainedModel):
    """ Expand from BertModel to handle image region features as input
    """

    def __init__(self, config):
        super(BertImgModel_gtToken, self).__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.encoder = CaptionBertEncoder(config)
        self.pooler = BertPooler(config)
        ...
        self.img_token_type_embeddings = nn.Embedding(2, self.config.hidden_size)
        self.img_pos_layer_norm = FusedLayerNorm(self.config.hidden_size, eps=1e-12)
        self.img_pos_linear = nn.Linear(7, self.config.hidden_size)
        # 问题中出现的实体则为0，未出现的实体为1
        self.img_embedding = nn.Linear(self.img_dim, self.config.hidden_size, bias=True)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if self.use_img_layernorm:
            self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.img_layer_norm_eps)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                position_ids=None, head_mask=None, img_feats=None, img_token_type_ids=None, img_pos_feat=None,
                encoder_history_states=None):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        ...
        if img_feats is not None:
            transformed_pos = self.img_pos_layer_norm(self.img_pos_linear(img_pos_feat))
            img_embedding_output = self.img_embedding(img_feats)
            img_type_embeddings = self.img_token_type_embeddings(img_token_type_ids)
            img_embedding_output = img_type_embeddings + img_embedding_output + transformed_pos
...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None, img_token_type_ids=None,
                       img_pos_feat=None,
...
class BertForImageCls(CaptionPreTrainedModel):
    """
    Bert for Image Captioning.
    """

    def __init__(self, config):
        super(BertForImageCls, self).__init__(config)
        self.config = config
        self.bert = BertImgModel(config)
        self.cls = BertOnlyMLMHead(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids, img_feat,
                input_mask=None, label=None,
                token_type_ids=None, position_ids=None, head_mask=None, encoder_history_states=None):
        outputs = self.bert(input_ids, img_feats=img_feat, attention_mask=input_mask,
                            position_ids=position_ids, token_type_ids=token_type_ids,
                            head_mask=head_mask,
                            encoder_history_states=encoder_history_states)
        pooled_output = outputs[1]
...
class BertForImageCaptioningAndCls(CaptionPreTrainedModel):
    ...
    def forward(self, *args, **kwargs):
        ...
        if is_decode:
            return self.generate_gen(*args, **kwargs)
        else:
            return self.encode_forward(*args, **kwargs)

    def encode_forward(self, input_ids, img_feat, attention_mask, input_mask=None, label=None,
                       token_type_ids=None, position_ids=None, head_mask=None,
                       is_training=True, encoder_history_states=None, is_gen=False, only_cls=False):
        if only_cls:
            outputs = self.bert(input_ids, img_feats=img_feat, attention_mask=input_mask,
                                position_ids=position_ids, token_type_ids=token_type_ids,
                                head_mask=head_mask,
            ...
            return loss_cls, matched
        if is_training:
            outputs = self.bert(input_ids, img_feats=img_feat, attention_mask=attention_mask,
                                position_ids=position_ids, token_type_ids=token_type_ids,
                                head_mask=head_mask,
                                encoder_history_states=encoder_history_states)
            sequence_output = outputs[0][:, 40:80]
            # num_masks_in_batch * hidden_size
            label_gen = input_ids[:, 40:80][:, 1:]
            label_gen = torch.cat((label_gen, torch.zeros((label_gen.size(0), 1), dtype=label_gen.dtype).cuda()),
                                  dim=-1)
            class_logits = self.cls(sequence_output)
            loss_gen = self.gen_criterion(class_logits.reshape(class_logits.size(0) * class_logits.size(1), -1),
                                          label_gen.reshape(-1))
            outputs = self.bert(input_ids, img_feats=img_feat, attention_mask=input_mask,
                                position_ids=position_ids, token_type_ids=token_type_ids,
                                head_mask=head_mask,
            ...
            loss_cls = loss_fct(logits.view(-1, self.num_labels), label.view(-1))
            pre = logits.max(dim=-1)[1]
            matched_qa = pre == label
            return loss_gen, loss_cls, matched_qa
        else:
            # 测试时，仅进行解码，一步步生成
            outputs = self.bert(input_ids, img_feats=img_feat, attention_mask=attention_mask,
                                position_ids=position_ids, token_type_ids=token_type_ids,
    ...
    def generate_gen(self, img_feat, attention_mask, label=None, input_mask=None, img_token_type_ids=None,
                     token_type_ids=None,
                     position_ids=None, head_mask=None, input_ids=None, max_length=None,
                     do_sample=None, num_beams=None, temperature=None, top_k=None, top_p=None,
                     repetition_penalty=None, bos_token_id=None, pad_token_id=None,
                     ...
                     add_od_labels=False, od_labels_start_posid=None,
                     use_cbs=False, fsm=None, num_constraints=None,
                     min_constraints_to_satisfy=None, is_gen=None, masked_pos=None
                     ):
        """ Generates captions given image features
        """
        assert is_decode
        batch_size = img_feat.shape[0]
        self.img_seq_len = img_feat.shape[1]
        self.max_seq_len = max_length
        self.mask_token_id = mask_token_id
        self.prev_encoded_layers = None
        # NOTE: num_keep_best is not equavilant to num_return_sequences
        ...
        if self.add_od_labels:
            # get od labels part from input_ids
            assert input_ids.shape[0] == batch_size
            od_label_ids = input_ids[:, self.max_seq_len * 2:]
            self.od_labels_len = input_ids.shape[1] - self.max_seq_len * 2
            input_ids = input_ids[:, :self.max_seq_len]
            input_ids = torch.cat((input_ids, torch.full(
                (batch_size, 1), bos_token_id, dtype=torch.long, device=img_feat.device
            )), dim=-1)
        else:
            self.od_labels_len = 0
            od_label_ids = None
        ...
        if position_ids is None:
            position_ids = torch.arange(self.max_seq_len * 2, dtype=torch.long, device=input_ids.device)
            posids_len = self.max_seq_len * 2
            if self.add_od_labels:
                od_labels_posids = torch.arange(
                    self.od_labels_start_posid,
                    self.od_labels_start_posid + self.od_labels_len, dtype=torch.long, device=input_ids.device)
        ...
        num_expand = num_beams * num_fsm_states * num_return_sequences
        self.od_label_ids = self._expand_for_beams(od_label_ids, num_expand)
        self.img_feats = self._expand_for_beams(img_feat, num_expand)
        ...
        if not use_cbs:
            if num_beams > 1:
                output = self._generate_beam_search_gen_SNLI(
                    input_ids,
                    cur_len,
                    max_length,
                    do_sample,
                ...
                output = self._generate_no_beam_search_gen_SNLI(
                    input_ids,
                    cur_len,
                    max_length,
                    do_sample,
...
class BertForImageCaptioning_gtToken(CaptionPreTrainedModel):
    """
    Bert for Image Captioning.
    """

    def __init__(self, config):
        super(BertForImageCaptioning_gtToken, self).__init__(config)
        self.config = config
        self.bert = BertImgModel_gtToken(config)
        self.cls = BertOnlyMLMHead(config)
        self.loss = BertCaptioningLoss(config)
        self.init_weights()
        # self.apply(self.init_weights)
    ...
    def forward(self, *args, **kwargs):
        ...
        if is_decode:
            if is_train:
                return self.generate_train(*args, **kwargs)
            elif is_gen:
                return self.generate_gen(*args, **kwargs)
            else:
                return self.generate(*args, **kwargs)
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, img_token_type_ids=None,
                       img_pos_feat=None, masked_ids=None,
                       token_type_ids=None, position_ids=None, head_mask=None,
                       is_training=True, encoder_history_states=None, is_gen=False):
        outputs = self.bert(input_ids, img_feats=img_feats, attention_mask=attention_mask,
                            position_ids=position_ids, token_type_ids=token_type_ids,
                            img_token_type_ids=img_token_type_ids, img_pos_feat=img_pos_feat,
                            head_mask=head_mask,
                            encoder_history_states=encoder_history_states)
        sequence_output = outputs[0][:, :masked_pos.shape[-1], :]
        ...
        if is_training:
            ...
            if is_gen:
                label = input_ids[:, 1:]
                label = torch.cat((label, torch.zeros((label.size(0), 1), dtype=label.dtype).cuda()), dim=-1)

                class_logits = self.cls(sequence_output)
                loss = self.gen_criterion(class_logits.reshape(class_logits.size(0) * class_logits.size(1), -1),
                                          label.reshape(-1))
                outputs = (loss, class_logits,) + outputs[2:]
                # sequence_output_masked = sequence_output[masked_pos == 1, :]
                # class_logits = self.cls(sequence_output_masked)
                # masked_ids = masked_ids[masked_ids != 0]  # remove padding masks
                # masked_loss = self.loss(class_logits.float(), masked_ids)
    ...
    def prepare_inputs_for_generation_gen(self, curr_ids, past=None):
        ...
        if past is None:
            ...
            position_ids = _remove_elements(self.full_position_ids, seq_start, seq_end)
            img_feats = self.img_feats
            img_token_type_ids = self.img_token_type_ids
            img_pos_feat = self.img_pos_feat
            ...
            start_pos = curr_ids.shape[1] - 1
            end_pos = start_pos + input_ids.shape[1]
            masked_pos = torch.zeros((input_ids.shape[0], 1), dtype=torch.int64)
            token_type_ids = _slice(self.full_token_type_ids, start_pos, end_pos)
            position_ids = _slice(self.full_position_ids, start_pos, end_pos)
        ...
        return {'input_ids': input_ids, 'img_feats': img_feats,
                ...
                'token_type_ids': token_type_ids, 'position_ids': position_ids,
                'is_training': False,
                'encoder_history_states': self.prev_encoded_layers, 'img_token_type_ids': img_token_type_ids,
                'img_pos_feat': img_pos_feat, 'is_gen': True}
    ...
    def generate(self, img_feats, attention_mask, masked_pos, img_token_type_ids=None, img_pos_feat=None,
                 token_type_ids=None,
                 position_ids=None, head_mask=None, input_ids=None, max_length=None,
                 do_sample=None, num_beams=None, temperature=None, top_k=None, top_p=None,
                 repetition_penalty=None, bos_token_id=None, pad_token_id=None,
                 ...
                 use_cbs=False, fsm=None, num_constraints=None,
                 min_constraints_to_satisfy=None, use_hypo=False,
                 decoding_constraint_flag=None, bad_ending_ids=None, is_train=None, is_gen=None
                 ):
        """ Generates captions given image features
        """
        assert is_decode
    ...
    def generate_train(self, img_feats, attention_mask, masked_pos, img_token_type_ids=None, img_pos_feat=None,
                       token_type_ids=None,
                       position_ids=None, head_mask=None, input_ids=None, max_length=None,
                       do_sample=None, num_beams=None, temperature=None, top_k=None, top_p=None,
                       repetition_penalty=None, bos_token_id=None, pad_token_id=None,
        ...
        output = self._generate_no_beam_search_train(
            input_ids,
            cur_len,
            max_length,
            do_sample,
    ...
    def generate_gen(self, img_feats, attention_mask, masked_pos, img_token_type_ids=None, img_pos_feat=None,
                     token_type_ids=None,
                     position_ids=None, head_mask=None, input_ids=None, max_length=None,
                     do_sample=None, num_beams=None, temperature=None, top_k=None, top_p=None,
                     repetition_penalty=None, bos_token_id=None, pad_token_id=None,
