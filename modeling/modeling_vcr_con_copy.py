...
def binary_to_mp(logit):
    ...
    logit = logit.reshape(-1, 4)  # group them into 4's
...
class ChunkAlign_CLS_dec5_4_copy_mask_5(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_copy_mask_5, self).__init__()
        ...
        self.enc_vocab_num = len(self.enc_toker)
        ...
        self.stop_words = ['a', 'is', 'am', 'are', 'the', 'an', 'it', 'has', 'very', 'and', 'his', 'her', 'there',
                           ...
                           'have', 'them', 'him', 'than', '[SEP]', '[PAD]', 'who']
    ...
    def get_copy_mask(self, attn_weight, input_ids, input_mask):
        ...
        for idx in range(batch_size):
            ...
            for id_index, id in enumerate(ids):
                token = self.enc_toker.decode(id)
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        encoder_hs_chosen = torch.gather(encoder_hs, 0,
                                         gather_index.unsqueeze(-1).unsqueeze(-1).repeat(1, encoder_hs.size(-2),
                                                                                         encoder_hs.size(-1)))
        input_ids_chosen = torch.gather(input_ids, 0, gather_index.unsqueeze(-1).repeat(1, input_ids.size(-1)))
        copy_mask_chosen = torch.gather(copy_mask, 0, gather_index.unsqueeze(-1).repeat(1, copy_mask.size(-1)))
        ...
        gpt_outputs = self.dec(input_ids=expl_ids, attention_mask=attn_mask, encoder_hidden_states=encoder_hs_chosen,
                               encoder_attention_mask=encoder_mask_chosen)
        ...
        cross_attention_weights = cross_attention_weights + copy_mask_chosen.unsqueeze(1)
        cross_attention_weights = torch.softmax(cross_attention_weights, dim=-1)

        context_vector = torch.matmul(cross_attention_weights, encoder_hs_chosen)
        ...
        one_hot = one_hot.scatter_(dim=-1, index=input_ids_chosen.unsqueeze(-1), value=1)[:, 1:, :]
    ...
    def evaluate(self, input_ids, img_feat, expl_ids, input_mask=None, label=None,
        ...
        expl_ids = expl_ids.reshape(ques_num, 4, -1)[:, 0, :]
        batch_size = encoder_hs_chosen.size(0)
        outputs = torch.full((batch_size, self.max_gen_len), fill_value=self.dec_toker.pad_token_id,
                             dtype=int).cuda()
        one_hot = torch.zeros(batch_size, hypo_len, self.enc_vocab_num).cuda()
        one_hot = one_hot.scatter_(dim=-1, index=input_ids_chosen.unsqueeze(-1), value=1)[:, 1:, :]
        one_hot = one_hot.unsqueeze(1).repeat(1, 3, 1, 1).reshape(batch_size, -1, self.enc_vocab_num)
        for i in range(matched_0.size(0)):
            if matched_0[i] == torch.tensor(True):
                ...
                for index in range(self.max_gen_len - 1):
                    ...
                    cross_attention_weights = torch.stack(cross_attention_weights).cuda().sum(dim=0)
                    cross_attention_weights = cross_attention_weights.sum(dim=1)
                    cross_attention_weights = cross_attention_weights + copy_mask_chosen[i].unsqueeze(0).unsqueeze(0)
                    cross_attention_weights = torch.softmax(cross_attention_weights, dim=-1)
                    context_vector = torch.matmul(cross_attention_weights, encoder_hs_chosen[i].unsqueeze(0))
        ...
        for i in range(matched_0.size(0)):
            if matched_0[i] == torch.tensor(True):
                ...
                beam_scorer = BeamSearchScorer_constrained(
                    ...
                    max_length=self.max_gen_len + b_rtnl_index,
                ...
                beam_output = self.beam_sample(input_ids=prompt_decoder_input.unsqueeze(0), beam_scorer=beam_scorer,
                                               ...
                                               copy_mask=copy_mask_chosen[i].unsqueeze(0),
                                               one_hot=one_hot[i].unsqueeze(0),
                                               max_length=self.max_gen_len + b_rtnl_index)
...
class ChunkAlign_CLS_dec5_4_copy_mask_5_1(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_copy_mask_5_1, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toker = enc_toker
        self.dec_toker = dec_toker
...
class ChunkAlign_CLS_dec5_4_copy_mask_5_1_1(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_copy_mask_5_1_1, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toker = enc_toker
        self.dec_toker = dec_toker
        ...
        self.stop_words = ['a', 'is', 'am', 'are', 'the', 'an', 'it', 'has', 'very', 'and', 'his', 'her', 'there',
                           'she', 'they', 'what', 'this', 'that', 'these', 'those', 'do', 'does', 'why', 'for',
                           'with', 'how', 'when', 'where', 'woman', 'man', 'women', 'men', 'some', 'while', 'did',
                           'done', 'about', 'will', 'all', 'into', 'being', 'like', 'look', 'therefore', 'isn', 'now',
                           'have', 'them', 'him', 'than', '[SEP]', '[PAD]', 'just', 'over', 'doing', 'feeling', 'which',
                           'because', 'not', 'trying', 'want', 'their', 'would', 'feel', 'make', 'feel', 'yes', 'said',
                           'from', 'who', 'going', 'more']
        self.p_gen = nn.Sequential(
            nn.Linear(self.dec.config.n_embd * 3, 1),
            nn.Sigmoid())
...
class ChunkAlign_CLS_dec5_4_copy_mask_5_1_2(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_copy_mask_5_1_2, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toker = enc_toker
        self.dec_toker = dec_toker
...
class ChunkAlign_CLS_dec5_4_copy_mask_5_2(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_copy_mask_5_2, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toker = enc_toker
        self.dec_toker = dec_toker
    ...
    def get_copy_mask(self, attn_weight, input_ids, input_mask, pre_keywords=None):
        ...
        total_token = []
        attn_weight_ans = []
        que_mask = torch.zeros_like(input_ids[0, 1:])
        for i in range(batch_size):
            e_qn_index = torch.nonzero(input_ids[i] == self.enc_toker.sep_token_id) - 1
            tmp_que_mask = que_mask.clone()
            tmp_que_mask[e_qn_index[0]: e_qn_index[1]] = 1
            attn_weight_ans.append(attn_weight[i] * tmp_que_mask.repeat(3))
            total_token.append((e_qn_index[1] - e_qn_index[0]) * 3)

        attn_weight_ans = torch.stack(attn_weight_ans)
        constraint_scores, constraint_index = torch.sort(attn_weight_ans, descending=True, dim=-1)
        constraint_ids = input_ids[:, 1:].unsqueeze(1).repeat(1, 3, 1).reshape(batch_size, -1)
        constraint_ids = torch.gather(constraint_ids, -1, constraint_index)
        total_token = torch.stack(total_token)
        mid_index = total_token // 2
