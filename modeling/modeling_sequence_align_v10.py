...
class ChunkAlign(nn.Module):
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        attn_weight = torch.stack(seq_outputs[2][-3:], dim=1).sum(1).sum(1)[:, 1:hypo_len, hypo_len:]
        img_num = input_mask[:, hypo_len:].sum(-1)
        img_num = torch.where(img_num == 1, torch.full_like(img_num, 2), img_num)
        attn_weight_dis = (1 - self.attention_thresh) / (img_num - 1)
        attn_weight_dis = attn_weight_dis.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, attn_weight.size(2) - 1)
        attn_weight_dis = torch.cat((torch.full((attn_weight_dis.size(0), attn_weight_dis.size(1), 1),
                                                self.attention_thresh).cuda(), attn_weight_dis), -1)
        attn_weight_dis = torch.where(attn_weight == 0, torch.zeros_like(attn_weight_dis), attn_weight_dis)
        attn_weight_dis_with_small = attn_weight_dis.masked_fill(attn_weight_dis == 0, 1e-9)
        attn_dis_entropy = -1 * (attn_weight_dis * torch.log2(attn_weight_dis_with_small)).sum(-1)
        attn_weight = attn_weight.masked_fill(attn_weight == 0, -1e5)
        attn_weight = nn.Softmax(dim=-1)(attn_weight)
        attn_weight_with_small = attn_weight.masked_fill(attn_weight == 0, 1e-9)
        attn_entropy = -1 * (attn_weight * torch.log2(attn_weight_with_small)).sum(-1)
        # 和均匀分布的熵的差值，越大越好
        attn_gate = F.sigmoid(self.gate_weight * (attn_dis_entropy - attn_entropy))
        chunk_align = chunk_align * attn_gate.unsqueeze(-1)
    ...
    def evaluate(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        word_mask = input_mask[:, 1:hypo_len].unsqueeze(1)
        word_mask = (1.0 - word_mask) * -10000.0
        word_mask = torch.cat((word_mask, word_mask), -1)
        for i, layer_module in enumerate(self.cls_layer):
            CLS_ensem = layer_module(self_chunk_align, CLS_ensem, word_mask)
        logits = self.classifier(CLS_ensem)
        pre = logits.max(dim=-1)[1]
...
class ChunkAlign_wo_gate(nn.Module):
    def __init__(self, global_enc, dec, seq_enc, dec_toker, beam_size, max_hypo):
        super(ChunkAlign_wo_gate, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.dec_toker = dec_toker
        self.dec = dec
...
class ChunkAlign_wo_gate_image(nn.Module):
    def __init__(self, global_enc, dec, seq_enc, dec_toker, beam_size, max_hypo):
        super(ChunkAlign_wo_gate_image, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.dec_toker = dec_toker
        self.dec = dec
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        encoder_hs = torch.cat((global_output[:, 1:], chunk_align), dim=1)
        encoder_mask = torch.cat((input_mask[:, 1:], input_mask[:, 1:hypo_len]), 1)
...
class ChunkAlign_CLS(nn.Module):
    def __init__(self, global_enc, seq_enc):
        super(ChunkAlign_CLS, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
...
class ChunkAlign_detach(nn.Module):
    def __init__(self, global_enc, dec, seq_enc, dec_toker, beam_size, max_hypo):
        super(ChunkAlign_detach, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.dec_toker = dec_toker
        self.dec = dec
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        encoder_hs = torch.cat((global_output, seq_outputs[0]), dim=1).detach()
        encoder_mask = torch.cat((input_mask, input_mask), 1)
...
class ChunkAlign_dec3(nn.Module):
    def __init__(self, global_enc, dec, seq_enc, dec_toker, beam_size, max_hypo):
        super(ChunkAlign_dec3, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.dec_toker = dec_toker
        self.dec = dec
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        encoder_hs = torch.cat((seq_outputs[0][:, 1:], global_output[:, 1:]), dim=1)
        encoder_mask = torch.cat((input_mask[:, 1:], input_mask[:, 1:]), 1)
...
class ChunkAlign_dec4(nn.Module):
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        encoder_hs = torch.cat((seq_outputs[0], global_output), dim=1).detach()
        encoder_mask = torch.cat((input_mask, input_mask), 1)
    ...
    def evaluate(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        encoder_hs = torch.cat((seq_outputs[0], global_output), dim=1)
        encoder_mask = torch.cat((input_mask, input_mask), 1)
        outputs = torch.full((expl_ids.size(0), self.max_gen_len), fill_value=self.dec_toker.pad_token_id,
                             dtype=int).cuda()
