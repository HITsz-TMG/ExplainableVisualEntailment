...
from .modeling_transfomres import Attention
...
class ChunkAlign_CLS_dec5_4_con_copy_mask_5(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_con_copy_mask_5, self).__init__()
        ...
        self.enc_vocab = self.enc_toker.vocab
        self.enc_vocab_num = self.enc_toker.vocab_size
        self.vocab_num = self.dec.vocab_size
        self.dec_vocab = self.dec_toker.encoder
        self.lm_head = nn.Linear(self.dec.config.n_embd, self.dec.vocab_size, bias=False)
        self.gen_criterion = torch.nn.NLLLoss(reduction='mean', ignore_index=self.dec_toker.pad_token_id)
        self.max_gen_len = 50
        self.beam_size = 5
        self.p_gen = nn.Sequential(
            nn.Linear(self.dec.config.n_embd * 3, 1),
            nn.Sigmoid())
    ...
    def get_src_to_tgt_vocab_conversion_matrix(self):
        ...
        src_to_tgt_conversion_matrix = torch.zeros(self.enc_vocab_num, self.vocab_num).cuda()
        src_vocab_items = self.enc_vocab.items()
        # Go over all (token, id) items in src vocab
        for src_token, src_id in src_vocab_items:
            token = self.dec_toker.tokenize(' ' + src_token)
            tgt_id = self.dec_toker.encode(token)[0]
            # tgt_id = self.dec_vocab.get('Ġ' + src_token, self.dec_toker.unk_token_id)
            src_to_tgt_conversion_matrix[src_id][tgt_id] = 1
        return src_to_tgt_conversion_matrix

    def bi_enc(self, input_ids, img_feat, input_mask, token_type_ids=None, position_ids=None, head_mask=None,
               encoder_history_states=None, offsets=None, chunk_attention_mask=None, gather_index=None):
        hypo_len = input_ids.size(1)
        ...
        attn_weight = attn_weight.sum(dim=0).squeeze(1)
        attn_weight += word_mask.squeeze(1)
        attn_weight = torch.softmax(attn_weight, dim=-1)

        return CLS_ensem, self_chunk_align, attn_weight, seq_outputs, global_output, chunk_hidden_states

    def get_copy_mask(self, attn_weight, input_ids, input_mask):
        batch_size = input_ids.size(0)
        hypo_len = input_ids.size(1)
        constraint_scores, constraint_index = torch.sort(attn_weight, descending=True, dim=-1)
        constraint_ids = input_ids[:, 1:].unsqueeze(1).repeat(1, 3, 1).reshape(batch_size, -1)
        constraint_ids = torch.gather(constraint_ids, -1, constraint_index)
        total_token = input_mask[:, 1:hypo_len].sum(-1) * 3
        mid_index = total_token // 2
        input_len = hypo_len - 1
        # 作为只有在推理过程中被关注的词才有机会被copy,不需要考虑[CLS]
        copy_mask = torch.full((batch_size, input_len * 3), fill_value=-10000).cuda()
        tokens = [[] for i in range(batch_size)]
        add_score_ids = [[] for i in range(batch_size)]
        for idx in range(batch_size):
            ids = constraint_ids[idx].tolist()
            index = constraint_index[idx].tolist()
            for id_index, id in enumerate(ids):
                ...
                if token in tokens[idx] or '#' in token or token in self.stop_words or len(token) <= 2:
                    continue
                tokens[idx].append(token)
                add_score_ids[idx].append(self.dec_toker.encode(' ' + token)[0])
                tmp_index = index[id_index] % input_len
                copy_mask[idx, tmp_index] = 0
                copy_mask[idx, tmp_index + input_len] = 0
                copy_mask[idx, tmp_index + input_len * 2] = 0
        return copy_mask, add_score_ids
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        CLS_ensem, self_chunk_align, attn_weight, seq_outputs, global_output, chunk_hidden_states = self.bi_enc(
            input_ids, img_feat, input_mask, token_type_ids,
            position_ids, head_mask, encoder_history_states,
            offsets, chunk_attention_mask, gather_index)
        copy_mask, add_score_ids = self.get_copy_mask(attn_weight=attn_weight, input_ids=input_ids,
                                                      input_mask=input_mask)

        logits = self.classifier(CLS_ensem)
        label = torch.argmax(label, -1)
        loss_cls_0 = self.cls_loss_fct(logits.view(-1, self.num_labels), label)
        ...
        gpt_outputs = self.dec(input_ids=expl_ids, attention_mask=attn_mask, encoder_hidden_states=encoder_hs,
                               encoder_attention_mask=encoder_mask, output_attentions=True)
        gpt_out = gpt_outputs[0]
        lm_logits = self.lm_head(gpt_out)
        lm_logits = torch.softmax(lm_logits, dim=-1)

        cross_attention_weights = gpt_outputs.cross_attentions
        ...
        cross_attention_weights = cross_attention_weights + copy_mask.unsqueeze(1)
        cross_attention_weights = torch.softmax(cross_attention_weights, dim=-1)

        context_vector = torch.matmul(cross_attention_weights, encoder_hs)
        tgt_embed = self.dec.wte(expl_ids)
        p_gen = self.p_gen(torch.cat((context_vector, gpt_out, tgt_embed), dim=-1))
        p_copy = 1 - p_gen

        # Get representation of src tokens as one hot encoding
        one_hot = torch.zeros(batch_size, hypo_len, self.enc_vocab_num).cuda()
        one_hot = one_hot.scatter_(dim=-1, index=input_ids.unsqueeze(-1), value=1)[:, 1:, :]
        one_hot = one_hot.unsqueeze(1).repeat(1, 3, 1, 1).reshape(batch_size, -1, self.enc_vocab_num)
        # p_copy from source is sum over all attention weights for each token in source
        p_copy_src_vocab = torch.matmul(cross_attention_weights, one_hot)
        # convert representation of token from src vocab to tgt vocab
        p_copy_tgt_vocab = torch.matmul(p_copy_src_vocab, self.src_to_tgt_vocab_conversion_matrix)

        logits_with_con = torch.add(lm_logits * p_gen, p_copy_tgt_vocab * p_copy)
        shift_logits = logits_with_con[:, :-1].contiguous()
        shift_logits = torch.log(shift_logits)
    ...
    def evaluate(self, input_ids, img_feat, expl_ids, input_mask=None, label=None,
        ...
        encoder_mask = torch.cat((input_mask[:, 1:hypo_len], input_mask[:, 1:hypo_len], input_mask[:, 1:hypo_len]), 1)
        outputs = torch.full((expl_ids.size(0), self.max_gen_len), fill_value=self.dec_toker.pad_token_id).cuda()
        batch_size = expl_ids.size(0)
        one_hot = torch.zeros(batch_size, hypo_len, self.enc_vocab_num).cuda()
        one_hot = one_hot.scatter_(dim=-1, index=input_ids.unsqueeze(-1), value=1)[:, 1:, :]
        one_hot = one_hot.unsqueeze(1).repeat(1, 3, 1, 1).reshape(batch_size, -1, self.enc_vocab_num)
        for i in range(matched_0.size(0)):
            if matched_0[i] == torch.tensor(True):
                ...
                for index in range(self.max_gen_len - 1):
                    gpt_outputs = self.dec(input_ids=tokens_to_add.unsqueeze(0),
                                           encoder_hidden_states=encoder_hs[i].unsqueeze(0),
                                           encoder_attention_mask=encoder_mask[i].unsqueeze(0), use_cache=True,
                                           past_key_values=past_key_values)
                    past_key_values = gpt_outputs[1]
                    gpt_out = gpt_outputs[0][:, -1:, :]
                    # 只取最后一个作为当前步的输出
                    lm_logits = self.lm_head(gpt_out)
                    lm_logits = torch.softmax(lm_logits, dim=-1)
                    ...
                    cross_attention_weights = torch.stack(cross_attention_weights).cuda().sum(dim=0)
                    cross_attention_weights = cross_attention_weights.sum(dim=1)
                    cross_attention_weights = cross_attention_weights + copy_mask[i].unsqueeze(0).unsqueeze(0)
                    cross_attention_weights = torch.softmax(cross_attention_weights, dim=-1)
                    context_vector = torch.matmul(cross_attention_weights, encoder_hs[i].unsqueeze(0))
                    tgt_embed = self.dec.wte(tokens_to_add.unsqueeze(0))
                    p_gen = self.p_gen(torch.cat((context_vector, gpt_out, tgt_embed), dim=-1))
                    p_copy = 1 - p_gen

                    # p_copy from source is sum over all attention weights for each token in source
                    p_copy_src_vocab = torch.matmul(cross_attention_weights, one_hot[i].unsqueeze(0))
                    # convert representation of token from src vocab to tgt vocab
                    p_copy_tgt_vocab = torch.matmul(p_copy_src_vocab, self.src_to_tgt_vocab_conversion_matrix)

                    logits_with_con = torch.add(lm_logits * p_gen, p_copy_tgt_vocab * p_copy)

                    gen_label = torch.argmax(logits_with_con, dim=-1).squeeze(-1)
            ...
            if matched_0[i] == torch.tensor(True):
                ...
                logits_processor = LogitsProcessorList(
                    [RepetitionPenaltyLogitsProcessor(penalty=self.repeat_penalty),
                     ...
                     MinLengthLogitsProcessor(7, eos_token_id=self.dec_toker.eos_token_id)])
                # instantiate logits processors
                logits_warper = LogitsProcessorList([TopKLogitsWarper(50)])
                decoder_input = expl_ids[i]
                b_rtnl_index = torch.nonzero((decoder_input == self.b_rtnl.unsqueeze(0)).to(torch.int64))[0, -1]
                prompt_decoder_input = decoder_input[:b_rtnl_index + 1]

                beam_output = self.beam_sample(input_ids=prompt_decoder_input.unsqueeze(0), beam_scorer=beam_scorer,
                                               encoder_hidden_states=encoder_hs[i].unsqueeze(0),
                                               encoder_mask=encoder_mask[i].unsqueeze(0),
                                               logits_processor=logits_processor,
                                               logits_warper=logits_warper, add_score_ids=add_score_ids[i],
                                               copy_mask=copy_mask[i].unsqueeze(0), one_hot=one_hot[i].unsqueeze(0))
    ...
    def beam_sample(
        ...
        while cur_len < max_length:
            gpt_outputs = self.dec(input_ids=input_ids, encoder_hidden_states=encoder_hidden_states_ex,
            ...
            cross_attention_weights = gpt_outputs.cross_attentions
            cross_attention_weights = torch.stack(cross_attention_weights).cuda().sum(dim=0)
            cross_attention_weights = cross_attention_weights.sum(dim=1)[:, -1:, :]
            cross_attention_weights = cross_attention_weights + copy_mask.unsqueeze(1)
            cross_attention_weights = torch.softmax(cross_attention_weights, dim=-1)
            context_vector = torch.matmul(cross_attention_weights, encoder_hidden_states_ex)
            tokens_to_add = input_ids[:, -1:]
            tgt_embed = self.dec.wte(tokens_to_add)
            p_gen = self.p_gen(torch.cat((context_vector, gpt_out, tgt_embed), dim=-1))
            p_copy = 1 - p_gen
            ...
            next_token_logits = torch.add(lm_logits * p_gen, p_copy_tgt_vocab * p_copy).squeeze(1)
...
class ChunkAlign_CLS_dec5_4_con_copy_mask_5_1(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_con_copy_mask_5_1, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toker = enc_toker
        self.dec_toker = dec_toker
    ...
    def get_copy_mask(self, attn_weight, input_ids, input_mask, pre_keywords=None):
        batch_size = input_ids.size(0)
        hypo_len = input_ids.size(1)
        constraint_scores, constraint_index = torch.sort(attn_weight, descending=True, dim=-1)
        constraint_ids = input_ids[:, 1:].unsqueeze(1).repeat(1, 3, 1).reshape(batch_size, -1)
        ...
        for idx in range(batch_size):
            ...
            for id_index, id in enumerate(ids):
                ...
                if token in tokens[idx] or '#' in token or token in self.stop_words or len(token) <= 2:
                    continue
                token_id = self.dec_toker.encode(' ' + token)[0]
                if pre_keywords is not None:
                    if token_id not in pre_keywords[idx]:
                        # 训练时，采用golden keywords
                        continue
                tokens[idx].append(token)
                add_score_ids[idx].append(token_id)
                tmp_index = index[id_index] % input_len
                copy_mask[idx, tmp_index] = 0
                copy_mask[idx, tmp_index + input_len] = 0
                copy_mask[idx, tmp_index + input_len * 2] = 0
        return copy_mask, add_score_ids, tokens

    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
                token_type_ids=None, position_ids=None, head_mask=None, encoder_history_states=None,
                offsets=None, chunk_attention_mask=None, gather_index=None, gpt_labels=None, pre_keywords=None):
        batch_size = input_ids.size(0)
        hypo_len = input_ids.size(1)
        ...
        CLS_ensem, self_chunk_align, attn_weight, seq_outputs, global_output, chunk_hidden_states = self.bi_enc(
            ...
            position_ids, head_mask, encoder_history_states,
            offsets, chunk_attention_mask, gather_index)
        copy_mask, add_score_ids, tokens = self.get_copy_mask(attn_weight=attn_weight, input_ids=input_ids,
                                                              input_mask=input_mask, pre_keywords=pre_keywords)
    ...
    def evaluate(self, input_ids, img_feat, expl_ids, input_mask=None, label=None,
        ...
        for i in range(matched_0.size(0)):
            if matched_0[i] == torch.tensor(True):
                ...
                if cur_len == self.max_gen_len:
                    outputs[i, -1].masked_fill_(cur_unfinished.to(dtype=torch.bool), self.dec_toker.eos_token_id)
        return outputs, matched_0, pre, tokens
...
class ChunkAlign_CLS_dec5_4_con_copy_mask_5_1_1(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_con_copy_mask_5_1_1, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toker = enc_toker
        self.dec_toker = dec_toker
...
class ChunkAlign_CLS_dec5_4_con_copy_mask_5_2(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty=1.0,
                 length_penalty=1.0,
                 constrained=1.0):
        super(ChunkAlign_CLS_dec5_4_con_copy_mask_5_2, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toker = enc_toker
        self.dec_toker = dec_toker
        ...
        self.p_gen = nn.Sequential(
            nn.Linear(self.dec.config.n_embd * 3, 1),
            nn.Sigmoid())
        self.p_gen_dec = nn.Sequential(
            nn.Linear(self.dec.config.n_embd * 3, 1),
            nn.Sigmoid())
        self.b_rtnl = torch.tensor(self.dec_toker.encode("<|b_rtnl|>")).cuda()
        self.e_rtnl = self.dec_toker.encode("<|e_rtnl|>")[0]
        self.e_qn = torch.tensor(self.dec_toker.encode("<|e_qn|>")).cuda()
    ...
    def cal_p(self, p_gen, p_gen_dec):
        p_copy = 1 - p_gen
        p_copy_dec = 1 - p_gen_dec
        p_gen_db = p_gen * p_gen_dec
        p_copy_db = p_copy * p_gen_dec
        p_copy_dec_db = p_copy_dec * p_gen
        p_sum = p_gen_db + p_copy_db + p_copy_dec_db
        return p_gen_db / p_sum, p_gen_db / p_sum, p_copy_dec_db / p_sum
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        self_attention = gpt_outputs.attentions
        self_attention = torch.stack(self_attention).cuda().sum(dim=0)
        self_attention = self_attention.sum(dim=1)
        self_attn_mask = attn_mask.unsqueeze(1)
        self_attn_mask = torch.where(self_attn_mask == 0, torch.full_like(self_attn_mask, fill_value=-1000),
                                     torch.zeros_like(self_attn_mask))
        self_attention += self_attn_mask
        context_vector_dec = []
        p_copy_dec_vocab = []
        for i in range(batch_size):
            decoder_input = expl_ids[i]
            e_qn_index = torch.nonzero((decoder_input == self.e_qn.unsqueeze(0)).to(torch.int64))[0, -1]
            self_attention_cur = self_attention[i, :, 2:e_qn_index]
            self_attention_cur = torch.softmax(self_attention_cur, dim=-1)
            dec_hs_cur = gpt_out[i, 2:e_qn_index, :]
            context_vector_cur = torch.matmul(self_attention_cur, dec_hs_cur)
            context_vector_dec.append(context_vector_cur)

            one_hot_dec = torch.zeros(e_qn_index - 2, self.vocab_num).cuda()
            one_hot_dec = one_hot_dec.scatter_(dim=-1, index=decoder_input[2:e_qn_index].unsqueeze(-1), value=1)
            p_copy_dec_vocab_cur = torch.matmul(self_attention_cur, one_hot_dec)
            p_copy_dec_vocab.append(p_copy_dec_vocab_cur)

        context_vector_dec = torch.stack(context_vector_dec)
        p_gen_dec = self.p_gen_dec(torch.cat((context_vector_dec, gpt_out, tgt_embed), dim=-1))
        p_copy_dec_vocab = torch.stack(p_copy_dec_vocab)

        p_gen_avg, p_copy_avg, p_copy_dec_avg = self.cal_p(p_gen, p_gen_dec)
        logits_with_con = torch.add(lm_logits * p_gen_avg, p_copy_tgt_vocab * p_copy_avg)
        logits_with_con = torch.add(logits_with_con, p_copy_dec_vocab * p_copy_dec_avg)
