...
from oscar.transformers.modeling_bert import BertConfig, load_tf_weights_in_bert, BERT_PRETRAINED_MODEL_ARCHIVE_LIST, \
    BertPreTrainedModel
...
class CaptionPreTrainedModel(BertPreTrainedModel):
    ...
    def _generate_no_beam_search_train(
            self,
            input_ids_raw,
            cur_len,
            max_length,
        ...
        while cur_len < max_length:
            ...
            input_ids = input_ids_raw[:, :cur_len + 1]
        ...
        if cur_len != max_length:
            pad = torch.zeros((gen_logits.size(0), max_length - cur_len, gen_logits.size(-1)),
                              dtype=gen_logits.dtype).cuda()
