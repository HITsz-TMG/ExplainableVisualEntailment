...
class BaseLine_cls_xe(nn.Module):
    def __init__(self, oscar, beam_size, num_labels):
    ...
    def forward(self, input_ids, img_feat, input_mask=None, label=None,
        ...
        loss_cls_0 = self.loss_fct(logits.view(-1, self.num_labels), label)
...
class BaseLine(nn.Module):
    def __init__(self, global_enc, dec, dec_toker, beam_size, num_labels):
    ...
    def evaluate(self, input_ids, img_feat, input_mask=None, label=None, attn_mask=None,
        ...
        logits = self.classifier(global_CLS)
        pre = logits.max(dim=-1)[1]
        matched_0 = pre == label.max(dim=-1)[1]

        encoder_hs = global_output
        encoder_mask = input_mask
        outputs = torch.full((encoder_hs.size(0), self.max_gen_len), fill_value=self.dec_toker.pad_token_id,
                             dtype=int).cuda()

        past_key_values = None
        cur_unfinished = outputs.new(outputs.size(0)).fill_(1)
        cur_len = 0
        tokens_to_add = torch.full((encoder_hs.size(0), 1), fill_value=self.dec_toker.bos_token_id).cuda().squeeze(-1)
        for index in range(self.max_gen_len - 1):
            gpt_out = self.dec(input_ids=tokens_to_add.unsqueeze(-1), encoder_hidden_states=encoder_hs,
                               encoder_attention_mask=encoder_mask, use_cache=True, past_key_values=past_key_values)
...
class ChunkAlign_CLS_enc4(nn.Module):
    ...
    def save_heat(self, input_ids, img_feat, input_mask=None, label=None,
        ...
        return matched_0, pre, torch.stack(seq_outputs[2])[:, :, :, :hypo_len, hypo_len:].squeeze(1)
...
class ChunkAlign_CLS_enc4_wo_chual(nn.Module):
    def __init__(self, global_enc, num_labels):
        super(ChunkAlign_CLS_enc4_wo_chual, self).__init__()
        self.global_enc = global_enc
        self.num_labels = num_labels
        self.cls_layer_num = 3
        self.cls_layer = nn.ModuleList([ClsLayer2(self.global_enc.config) for _ in range(self.cls_layer_num)])
    ...
    def forward(self, input_ids, img_feat, input_mask=None, label=None,
        ...
        self_chunk_align = global_hypo.contiguous()

        word_mask = input_mask[:, 1:hypo_len].contiguous().unsqueeze(1)
        word_mask = (1.0 - word_mask) * -10000.0
        for i, layer_module in enumerate(self.cls_layer):
            CLS_ensem, attn_weight = layer_module(self_chunk_align, CLS_ensem, word_mask)
        logits = self.classifier(CLS_ensem)
        label = torch.argmax(label, -1)
        loss_cls_0 = self.cls_loss_fct(logits.view(-1, self.num_labels), label)
        pre = logits.max(dim=-1)[1]
...
class ChunkAlign_CLS_enc4_wo_reasoning(nn.Module):
    def __init__(self, global_enc, seq_enc, num_labels):
        super(ChunkAlign_CLS_enc4_wo_reasoning, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.cls_ensemble = nn.Linear(self.global_enc.config.hidden_size + self.seq_enc.config.hidden_size,
                                      self.global_enc.config.hidden_size)
...
class ChunkAlign_CLS_dec5_4_gen(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty, length_penalty,
                 constrained, stop_words_index):
        super(ChunkAlign_CLS_dec5_4_gen, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
...
class ChunkAlign_CLS_dec5_4_wo_chual_test_beam(nn.Module):
    def __init__(self, global_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty, length_penalty,
                 constrained, stop_words_index):
        super(ChunkAlign_CLS_dec5_4_wo_chual_test_beam, self).__init__()
        self.global_enc = global_enc

        self.enc_toekr = enc_toker
        self.dec_toker = dec_toker
        self.dec = dec
    ...
    def test_beam(self, input_ids, img_feat, expl_ids, input_mask=None, label=None,
        ...
        for idx in range(batch_size):
            ids = constraint_ids[idx].tolist()
            for id in ids:
                token = self.enc_toekr.decode(id, skip_special_tokens=True)
                token = token.replace('.', '').replace(',', '')
...
class ChunkAlign_CLS_dec5_4_wo_reasoning_test_beam(nn.Module):
    def __init__(self, global_enc, seq_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty, length_penalty,
                 constrained, stop_words_index):
        super(ChunkAlign_CLS_dec5_4_wo_reasoning_test_beam, self).__init__()
        self.global_enc = global_enc
        self.seq_enc = seq_enc
        self.enc_toekr = enc_toker
        self.dec_toker = dec_toker
...
class ChunkAlign_CLS_dec5_4_wo_chual_reasoning(nn.Module):
    def __init__(self, global_enc, dec, enc_toker, dec_toker, num_labels, repeat_penalty, length_penalty,
                 constrained, stop_words_index):
        super(ChunkAlign_CLS_dec5_4_wo_chual_reasoning, self).__init__()
        self.oscar = global_enc
        self.enc_toekr = enc_toker
        self.dec_toker = dec_toker
        self.dec = dec
...
class CSA_woCHUAL(nn.Module):
    def __init__(self, global_enc, dec, dec_toker, num_labels):
        super(CSA_woCHUAL, self).__init__()
        self.oscar = global_enc
        self.dec_toker = dec_toker
        self.dec = dec
    ...
    def forward(self, input_ids, img_feat, expl_ids, input_mask=None, label=None, attn_mask=None,
        ...
        global_hypo = global_output[:, 1:hypo_len].contiguous()
        word_mask = input_mask[:, 1:hypo_len].unsqueeze(1)
        word_mask = (1.0 - word_mask) * -10000.0

        attn_weight = []
        for i, layer_module in enumerate(self.cls_layer):
            CLS_ensem, attn_weight_layer = layer_module(global_hypo, global_CLS, word_mask)
            attn_weight.append(attn_weight_layer)
