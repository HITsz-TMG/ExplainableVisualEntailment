        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i == self.cross_chunk_attention_layers[0]:
                ...
                if i == self.cross_modal_layers[0]:
                    # 图片部分只看到自己
                    assert attention_mask.size(2) == attention_mask.size(3) == hypo_len + img_len
...
class SeqAlign_pretrain_v2_align_only(nn.Module):
    def __init__(self, seq_enc, config):
        super(SeqAlign_pretrain_v2_align_only, self).__init__()
...
class SeqAlign_pretrain_v2_align_only_vcr(nn.Module):
    def __init__(self, seq_enc, config):
        super(SeqAlign_pretrain_v2_align_only_vcr, self).__init__()
        self.seq_enc = seq_enc
        self.cls_loss_fct = CrossEntropyLoss(reduction='mean', ignore_index=0)
        self.cls = BertOnlyMLMHead(config)
        self.loss = BertCaptioningLoss(config)
    ...
    def forward(self, input_ids, img_feat, total_label, img_mask, input_mask,
        ...
        return align_loss, correct, total_sum
...
class SeqAlign_pretrain_v2_align_mask_only(nn.Module):
    def __init__(self, seq_enc, config):
        super(SeqAlign_pretrain_v2_align_mask_only, self).__init__()
        self.dropout = nn.Dropout(0.3)
        self.seq_enc = seq_enc
        self.cls_loss_fct = CrossEntropyLoss(reduction='mean', ignore_index=0)
        self.cls = BertOnlyMLMHead(config)
